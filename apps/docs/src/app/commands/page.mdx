...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>`--tailscale-http`</td>
      <td>Share on the tailnet over plain HTTP, addressed by the node's tailnet IP. For tailnets that cannot enable MagicDNS. Cannot be combined with `--funnel`.</td>
    </tr>
    <tr>
      <td>`--funnel`</td>
