...
`pnpm dev` at the root runs turbo, which runs `portless` in each package. Portless detects both `turbo.json` and `turbo.jsonc`, then detects the package manager and runs `pnpm run dev:app` through the proxy. No Turborepo config changes are needed.
