...
`pnpm dev` at the root runs turbo, which runs `portless` in each package. Portless detects the package manager and runs `pnpm run dev:app` through the proxy. No `turbo.json` or `turbo.jsonc` changes are needed.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>`PORTLESS_TAILSCALE_HTTP`</td>
      <td>Set to `1` to share on the tailnet over plain HTTP (no MagicDNS needed)</td>
      <td>off</td>
    </tr>
    <tr>
