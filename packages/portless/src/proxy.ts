        ...
        routes.length > 0
          ? `<div class="section"><p class="label">Active apps</p><ul class="card">${routes
              .map((r) => {
                ...
                return `<li><a href="${escapeHtml(formatUrl(r.hostname, proxyPort, isTls, r.pathPrefix))}" class="card-link"><span class="name">${displayName}</span><span class="meta"><code class="port">127.0.0.1:${escapeHtml(String(r.port))}</code><span class="arrow">${ARROW_SVG}</span></span></a></li>`;
              })
              .join("")}</ul></div>`
